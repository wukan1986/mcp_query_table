...
async def main() -> None:
    ...
    df = await query(page, '收盘价>50元的港股', query_type=QueryType.HKStock, max_page=3, site=Site.THS)
    print(df.to_markdown())
    df = await query(page, '年初至今收益率前50', query_type=QueryType.Fund, max_page=3, site=Site.TDX)
    print(df.to_csv())
    # TODO 东财翻页要提前登录
